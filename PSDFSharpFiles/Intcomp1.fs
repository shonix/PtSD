module PSDExercises.Intcomp1_fsx
...
let rec lookup (env : (string * expr) list) x =
    match env with 
...
let rec eval e (env : (string * expr) list) : int =
    match e with
    | CstI i            -> i    
    | Var x             -> eval (lookup env x) env
    | Let(bind, exp) -> 
      let env1 = bind @ env // add new additions to env
      eval exp env1         // evaluate in the new env
...
// Run the translator over given sinstr list :D
