// File microJ/Parse.fs

// Lexing and parsing of micro-Java programs using fslex and fsyacc
...
let fromString (str : string) : program =
  ...
  try 
    JPar.Main JLex.Token lexbuf
  with 
    | exn -> let pos = lexbuf.EndPos
             fatal (sprintf "%s near line %d, column %d\n" (exn.Message) (pos.Line+1) pos.Column)
...
let fromFile cmdL =
  ...
  try 
    ...
    try
      let abs = JPar.Main JLex.Token lexbuf
      abs
    with
    | exn -> let pos = lexbuf.EndPos
