...
class Machine {
  ...
  final static int 
    CSTI = 0, ADD = 10, SUB = 11, MUL = 12, DIV = 13, MOD = 14, 
    EQ = 20, LT = 21, NOT = 22, 
    DUP = 30, SWAP = 31, 
    LDI = 40, STI = 41, 
    GETBP = 60, GETSP = 61, INCSP = 62, 
    GOTO = 70, IFZERO = 71, IFNZRO = 72, CALL = 80, TCALL = 81, RET = 86, 
    PRINTI = 90, PRINTNL = 94, 
    LDARGS = 100,
    STOP = 101;
  ...
  static int execcode(int[] p, int[] s, int[] iargs, boolean trace) {
    ...
    for (;;) {
      ...
      case LDARGS:
        int n = p[pc++];; // Number of expected arguments.
        if (n != iargs.length) {
          System.out.print("Program expects " + n + " argument(s), but got " + iargs.length + ".");
          return 0;
        }
