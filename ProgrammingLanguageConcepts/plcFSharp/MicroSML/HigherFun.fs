// Interpretating micro-SML
   
// The language is called micro-SML because it has some inspiration
// from Standard ML including generative exceptions and the
...
let createNumGen() =
  let num = ref 0
  let newNumGen() = num.Value <- num.Value + 1; num.Value
  newNumGen
...
and closure =
  Closure of string * string * expr<typ,typescheme> * value env // (f, x, fBody, fDeclEnv)
...
let rec ppValue = function
  ...
  | List l -> let elems = List.map ppValue l
              "[" + (String.concat "," elems) + "] "
  | ClosureRef closRef ->
    match closRef.Value with
    | Closure(f,x,fBody,fDeclEnv) -> "Fn@" + f + "("+ ((List.length fDeclEnv).ToString()) + ") "

let ppAnswer = function
    Result r -> ppValue r
  | Abort s -> s
...
let rec evalExpr (env : value env) (e : expr<typ,typescheme>)
  ...
  | Prim1(ope,e1,_) ->
    evalExpr env e1 (fun v1 ->
                     ...
                     | ("print",_) -> printf "%s " (ppValue v1); cont v1 // Polymorphic print.
                     | ("hd",_) -> match v1 with
...
and evalValdecs (env:value env) (ts:valdec<typ,typescheme> list) (body: expr<typ,typescheme>)
                (cont: value -> answer) (econt: value -> answer) : answer =
  match ts with
  | [] -> evalExpr env body cont econt                    
  | Fundecs fs :: ts' -> 
    let closureRefs = List.map (fun (f,x,fBody,_) -> (f, ref (Closure(f,x,fBody,env)))) fs
    let updClosRef (f,closRef: closure ref) =
      (match closRef.Value with
       | Closure(f,x,fBody,_) -> 
         ...
         (closRef.Value <- Closure(f,x,fBody,fBodyEnv)))
  ...
  | Exndec(ExnVar x)::ts' -> evalValdecs ((x,Int (freshExnName())) :: env) ts' body cont econt
and evalProg (env:value env) (t:program<typ,typescheme>) : answer =
  match t with
...
let check e =
  let rec check' (e : expr<'a,'b>) (env : string list) : bool =
    match e with
    | CstI (i,_) -> true
    | CstB (b,_) -> true
    | CstN _     -> true
  ...
  and checkValdec' (b : bool, env : string list) (valdec : valdec<'a,'b>) : bool * string list =
    match valdec with
    |  Fundecs fs -> 
      let fEnv = List.foldBack (fun (f,_,_,_) env -> f::env) fs env (* fBody may recursively call f *)
      (List.foldBack (fun (_,x,fBody,_) acc -> check' fBody (x::fEnv) && acc) fs b,  (* Only include x for function f *)
       fEnv)
    | Valdec(x,eRhs,_) -> (check' eRhs env,x::env)
    | Exndec(ExnVar x) -> (b, x::env)
  check' e []  
